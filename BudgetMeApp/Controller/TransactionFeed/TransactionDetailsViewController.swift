...
class TransactionDetailsViewController: UIViewController {
  ...
  func configureView() {
    ...
    transactionSubSourceLabel.text = transaction.sourceSubType.map(\.rawValue)
    ...
    spendingCategoryLabel.text = transaction.spendingCategory.map(\.rawValue)
  }

  func sourceTransactionAbbreviation(transaction: STTransactionFeed) -> String {
    var abbrev = ""

    let sourceComponents = transaction.source.map(\.rawValue)?.components(separatedBy: "_")
